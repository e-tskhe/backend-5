...
if ($errors['email']) {
    ...
    $messages[] = '<div class="error">Ошибка в поле email.</div>';
}
if ($errors['birthdate']) {
    setcookie('birthdate_error', '', 100000);
    setcookie('birthdate_value', '', 100000);

    $messages[] = '<div class="error">Ошибка в дате рождения.</div>';
}
if ($errors['gender']) {
...
if ($errors['bio']) {
    ...
    $messages[] = '<div class="error">Ошибка в поле биографии.</div>';
}
if ($errors['agreement']) {
